"""Alpha 101_049: Slope comparison with threshold signal.

Formula: condition(lt(sub(div(sub(delay({disk:close},20),delay({disk:close},10)),10),div(sub(delay({disk:close},10),{disk:close}),10)),mul(-1,0.1)),1,mul(mul(-1,1),sub({disk:close},delay({disk:close},1))))

Conditional factor based on multi-period slope comparison with -0.1 threshold.
"""
...
class alpha_101_049(BaseSignal):
    role = SignalRole.ALPHA
    """Alpha 101_049: Slope comparison with threshold.

    Returns 1 when slope difference is below -0.1, else negative daily change.
    """
